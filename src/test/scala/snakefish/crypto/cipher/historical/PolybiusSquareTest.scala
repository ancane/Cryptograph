...
class PolybiusSquareTest extends BaseTest {
  ...
  ".apply(key: CharSequence, alphabet, substitutes)" should "create Polibius square instance from provided arguments" in {
    ...
    square.square must equal(Array(Array('q', 'w', 'e', 'r', 't'), 
                                   ...
                                   Array('l', 'm', 'n', 'o', 'p'),
                                   Array('s', 'u', 'v', 'x', 'z')))
    square.substitutes must contain only(('j' -> 'i'))
  }
  
  ".apply(key: Long, alphabet, substitutes)" should "create Polibius square instance from provided arguments" in {
